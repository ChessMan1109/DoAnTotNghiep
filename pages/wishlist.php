...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wishlist'])) {
    $phoneId = $_POST['wishlist'];
    $_SESSION['wishlist'][] = $phoneId;
}
...
if (isset($_SESSION['wishlist']) && !empty($_SESSION['wishlist'])) {
    ...
    if (!empty($wishlistIds)) {
        $sql = "SELECT phones.ID, phones.Name, phones_images.ImageURL
                FROM phones
                JOIN phones_images ON phones.ID = phones_images.phoneID
                WHERE phones.ID IN ($wishlistIds)";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td><form action='remove_wishlist.php' method='POST'><input type='hidden' name='phoneID' value='" . $row['ID'] . "'><button type='submit' class='remove-button' style='background-color: transparent; border: none; cursor: pointer;'><i class='fa-solid fa-xmark'></i></button></form></td>";
            ...
            echo "<h3 style='font-size: 17px; font-weight: 400; margin-top: 30px;'>Không có sản phẩm nào được thêm vào danh Điện thoại yêu thích</h3>";
        }
    } else {
        echo "<h3 style='font-size: 17px; font-weight: 400; margin-top: 30px;'>Không có sản phẩm nào được thêm vào danh Điện thoại yêu thích</h3>";
    }
