...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['compare'])) {
    $phoneId = $_POST['compare'];
    $_SESSION['compare'][] = $phoneId;
}
...
if (isset($_SESSION['compare']) && !empty($_SESSION['compare'])) {
    ...
    if (!empty($compareIds)) {
        $sql = "SELECT phones.ID, phones.Name, phones.Price, phones.Discount, phones.Description, phones_images.ImageURL
                FROM phones
                JOIN phones_images ON phones.ID = phones_images.phoneID
                WHERE phones.ID IN ($compareIds)";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td><a id='seemore-compare' href='phone_detail.php?phoneID={$row['ID']}'>Xem chi tiết</a></td>";
                echo "<td><form action='remove_compare.php' method='POST'><input type='hidden' name='phoneID' value='" . $row['ID'] . "'><button type='submit' class='remove-button' style='background-color: transparent; border: none; cursor: pointer;'><i class='fa-solid fa-xmark'></i></button></form></td>";
                echo "</tr>";
            }
            echo "</table>";
        }
