...
if (isset($_POST['checkout'])) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        foreach ($_SESSION['cart'] as $item) {
            $subtotal = $item['phone_price'] * $item['quantity'];
        ...
        if ($result) {
            ...
            foreach ($_SESSION['cart'] as $item) {
                ...
                $sql = "INSERT INTO order_items (OrderID, PhoneID, PhoneName, PhonePrice, Quantity)
                        VALUES ('$order_id', '$phoneID', '$phoneName', '$phonePrice', '$quantity')";
            ...
            try {
                ...
                $mail->Username = 'quangminh456456@gmail.com'; // Tên đăng nhập SMTP
                $mail->Password = 'dcihithtorqijeul'; // Mật khẩu đăng nhập SMTP
                $mail->SMTPSecure = 'tls'; // Giao thức bảo mật
                $mail->Port = 587; // Cổng SMTP

                // Cấu hình email
                $mail->CharSet = 'UTF-8';
                $mail->setFrom('quangminh456456@gmail.com', 'Good Phone'); // Địa chỉ email và tên người gửi
                $mail->addAddress($email, $fname . ' ' . $lname); // Địa chỉ email và tên người nhận
                $mail->isHTML(true); // Gửi email dạng HTML
                $mail->Subject = 'Xác nhận đơn hàng từ Good Phone'; // Tiêu đề email

                // Nội dung email
                $mail->Body = "Chào $fname $lname,<br><br>
                            Cảm ơn bạn đã đặt hàng từ Good Phone.<br>
                ...
                if ($mail->send()) {
                    ...
                    echo '<a href="index_home.php">Tiếp tục mua sắm</a>';
                    echo '</div>';
                } else {
